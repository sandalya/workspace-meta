...
~~chkp.py auto-backlog-suggest (2026-05-15)::Додати у chkp.py другий Haiku-call після основного~~: дивиться на сесію (HOT.md з результату першого call + what_done/next_step/context) + активні пункти BACKLOG.md і пропонує які можливо закриваються (closed-as-done або closed-as-obsolete). UX: y/n/edit/skip перед apply_backlog_flags, default n після 30s timeout. Прапор --no-backlog-suggest для opt-out на дрібних chkp. Помилки Haiku не блокують chkp (fall-through з warning). 8 нових pytest. Корінь — systematic gap: AI у чаті не звіряє беклог з сесією перед chkp, тому --backlog-strike не передається навіть коли мав би. Документовано: 04.05 filter-repo household_agent (commit 0674dd4, chkp без strike), 15.05 (3 obsolete пункти знайдені руками за одну сесію). CC бриф у попередній сесії з лінком. Пріоритет: P2 — обхід є (ручний strike в наступному chkp), але дрейф беклогу регулярний. Розмір: 1-2h CC + smoke test

~~token_tracker write-side у sam/insilver/abby/ed/meggy (2026-05-15)~~::TokenTracker інстанціюється у main.py всіх ботів, але .log_call() не викликається на AI-зверненнях. Sam має /stats UI що завжди порожній. Підключити write-side у точках client.messages.create() / Anthropic API виклики. Розмір: 10-30 хв на проєкт x 5 = 1-3h. Пріоритет: P3 — Anthropic Console показує total, per-bot облік потрібен тільки якщо вирішиш виставляти Владу окремий рахунок або порівнювати моделі

~~### chkp.py pytest extension — баги audit-сесії 06.05 (2026-05-06)~~

Розширити meta/chkp/tests/ існуючу інфру (test_no_hallucination.py, conftest з вчорашньої сесії 05.05) новими кейсами на 3 виявлені сьогодні класи багів strikethrough логіки:

1. silent-skip при невідомій секції у --backlog-add → exit 0 + skipped count, реальна втрата записів. Має бути exit 1 + повідомлення "section X not found, valid: A, B, C".

2. multi-match без попередження у --backlog-strike → str.replace(,1) бере перший збіг навіть якщо їх 2+. Має бути warning + потребувати --force або унікальну підстроку.

3. replace(,1) hit example first → якщо у BACKLOG є приклад заголовка у > Приклад: блоці перед реальним пунктом, страйкається приклад. Сесія 06.05 — П.1 voice reference застрайкало рядок 14 замість 57. Фікс пов'язаний з #2.

4. (опціонально) ~~closed~~ парсинг — табличний тест на ACTIVE vs CLOSED заголовки, regression на баг "AI повертає closed як TODO" з рядка 21 BACKLOG.

Передумова: інфра з 05.05 (--dry-run, CHKP_WORKSPACE env, test fixtures) вже готова. Розмір: ~1-2 години.

Пріоритет: P2. ROI скромний (chkp падає рідко), але знімає страх чіпати chkp.py. Контекст: сесія 2026-05-06 (BACKLOG audit) виявила що поточних тестів недостатньо — три нових класи багів спливли за одну сесію.

~~### chkp.py replace(,1) bug — strike перший збіг замість унікального (2026-05-06)~~

Симптом: `chkp ... --backlog-strike "## Foo"` страйкає перший рядок з підстрокою, навіть якщо це приклад у > Приклад: блоці. Сьогодні через це П.1 застрайкало приклад на рядку 14 замість реального заголовка на рядку 57.

Фікс варіанти: (а) replace all замість replace(,1), (б) у validate_backlog_flags попереджати якщо grep -c "<substr>" > 1, (в) комбо: warn + потребує --force для multi-match.

Розмір: ~30 хв коду + pytest кейс у meta/chkp/tests/. Після фіксу страйкати треба буде унікальною підстрокою або з префіксом регіону.
